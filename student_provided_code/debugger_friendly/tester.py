...
TIME_LIMIT = 30  # seconds - Note, if you turn on Verbose Logging
...
class GliderSimulator:
    ...
    def simulate_without_steering(self, params):
        ...
        target = glider.glider(params['target_x'], params['target_y'],
        ...
        if PLOT_PARTICLES == True:
            ...
            X1, Y1, X2, Y2 = (-WINDOW_SIZE, -WINDOW_SIZE, WINDOW_SIZE, WINDOW_SIZE)  # WINDOW_SIZE is 300
        ...
        try:
            while steps < params['max_steps']:
                ...
                result = marsglider.estimate_next_pos(target_height, target_meas, studentMapFunc, other_info)
    ...
    def simulate_with_steering(self, params):
        ...
        try:
            while steps < params['max_steps']:
                target_meas = target.sense()
                target_height = target.get_height()
                result = marsglider.next_angle(target_height, target_meas, studentMapFunc, other_info)
                if len(result) == 3:
                ...
                print(separation)
                print(("Step: {} Separation is {}".format(steps, separation)))
...
s = GliderSimulator()
# for part A
s.simulate_without_steering(GLOBAL_PARAMETERS[1])
# for part B
s.simulate_with_steering(GLOBAL_PARAMETERS[1])
